...
def ntlm_check(host, username="A", password="A", domain="", hashes=None):
    ...
    if cr_tpdu['Type'] == TYPE_RDP_NEG_FAILURE:
        ...
        rdp_failure.dump()
        error("Server doesn't support PROTOCOL_HYBRID, hence we can't use CredSSP to check credentials")
        return
    else:
